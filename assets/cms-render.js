...
// dan post.html (khusus artikel detail + related posts).
...
/** Amankan URL dari admin: hanya http(s), mailto, tel, atau path relatif. */
function cmsSafeUrl(u) {
    const s = String(u || '').trim();
    if (/^(https?:|mailto:|tel:)/i.test(s)) return s;
    if (/^[\w-]+(\.[\w-]+)+(\/|$)/.test(s)) return 'https://' + s;
    return s.startsWith('#') || s.startsWith('uploads/') ? s : '#';
}

/** Blok media (gambar ATAU video) untuk About Us, Disclaimer & Footer. media = {type,src,caption} */
function cmsMediaHtml(media, cls) {
    if (!media || !media.src) return '';
    const src = cmsEsc(media.src);
    const isVideo = media.type === 'video' || (media.type !== 'image' && /\.(mp4|webm|mov|m4v)(\?|#|$)/i.test(media.src));
    const vtype = /\.webm(\?|#|$)/i.test(media.src) ? 'video/webm' : 'video/mp4';
    const cap = media.caption ? `<figcaption class="text-[9px] text-slate-400 mt-1 text-center">${cmsEsc(media.caption)}</figcaption>` : '';
    const inner = isVideo
        ? `<video controls playsinline preload="metadata" class="w-full rounded-xl bg-slate-900 aspect-video"><source src="${src}" type="${vtype}">Browser Anda tidak mendukung pemutar video.</video>`
        : `<img src="${src}" alt="${cmsEsc(media.caption || '')}" loading="lazy" class="w-full rounded-xl object-cover border border-slate-200">`;
    return `<figure class="${cmsEsc(cls || '')}">${inner}${cap}</figure>`;
}
...
function renderFooter() {
    ...
    const mediaEl = document.getElementById('footerMedia');
    ...
    if (mediaEl) mediaEl.innerHTML = cmsMediaHtml(f.media);

    if (linksEl) {
        linksEl.innerHTML = (f.socialLinks || []).map(s => `
            <a href="${cmsEsc(cmsSafeUrl(s.url))}" target="_blank" rel="noopener noreferrer" class="w-9 h-9 rounded-xl bg-slate-50 hover:bg-brand-blue hover:text-white border border-slate-200 flex items-center justify-center text-slate-700 transition-all shadow-sm" title="${cmsEsc(s.platform)}">
...
// ----------------------------------------------------------
// BLOG - halaman detail (post.html?id=N)
// ----------------------------------------------------------
    ...
    const mediaBlock = post.hasVideo ? `
        <figure class="my-4 -mx-0.5">
            <video controls preload="metadata" poster="${cmsEsc(post.secondaryImage)}" class="w-full rounded-xl aspect-video bg-slate-900">
                <source src="${cmsEsc(post.video)}" type="${/\.webm(\?|#|$)/i.test(post.video) ? 'video/webm' : 'video/mp4'}">
                Browser Anda tidak mendukung pemutar video.
            </video>
            <figcaption class="text-[10px] text-slate-400 mt-1.5 text-center">Video pendukung artikel.</figcaption>
        </figure>` : `
        <figure class="my-4 -mx-0.5">
